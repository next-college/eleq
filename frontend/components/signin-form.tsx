...
export function LoginForm() {
  ...
  const handleGoogleLogin = async () => {
    try {
      setIsLoading(true);
      setError("");
      // Next-auth handles the external redirect to Google
      // After success, it will automatically redirect to /dashboard
  ...
  const handleCredentialLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await signIn("credentials", {
        // IMPORTANT: Your route.ts uses 'username' as the key for the email field
        username: email,
        password: password,
        redirect: false,
      });
      ...
      if (result?.ok) {
        // Success: Redirect to dashboard and refresh the server components (like Navbar)
        ...
        <form onSubmit={handleCredentialLogin} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              style={{ backgroundColor: "#FFFFFF", color: "#540B26" }}
            />
          </div>
          <div className="space-y-2">
